...
def has_instrument_rating(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    if (student[7] is not None) and (student[7] != ''):
        time = utils.str_to_time(student[7])
        if time < takeoff:
            return True
        else: 
            return False
...
def has_advanced_endorsement(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    if (student[8] is not None) and (student[8] != ''):
        time = utils.str_to_time(student[8])
        if time < takeoff:
            return True
        else:
            return False
...
def has_multiengine_endorsement(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    if (student[9] is not None) and (student[9] != ''):
        time = utils.str_to_time(student[9])
        if time < takeoff:
            return True
        else:
            return False
...
#    Student,VMC,Pattern,Day,2000,5,20,8
#    cert, area, instructed, vfr, daytime,
#    (1,'Pattern',False,False,True,minimums)
    temp_rows = []    
    for item in minimums:
        if cert  <= 0 and instructed == False:
            return None
        else: 
            if item[2] == 'Any' or item[2] == area:
                if (item[3] == 'Day' and daytime == True) or (item[3] == 'Night' and daytime == False):
                    if (item[1] == 'VMC' and  vfr == True):
                        if item[0] == 'Dual' and instructed == True:
                            temp_rows.append(item)
                        if item[0] == '50 Hours' and cert == 3:  
                            temp_rows.append(item)
                        if item[0] == 'Certified' and cert >=2: 
                            temp_rows.append(item)
                        if item[0] == 'Student' and cert >=1:  
                            temp_rows.append(item) 
                    elif vfr == False:
                        if item[0] == 'Dual' and instructed == True:
                            temp_rows.append(item)
    ...
    # We then take the best values from all of these matches.
    # That means the least ceiling and visibility and the strongest wind
    # and crosswind.
    print("temp",temp_rows)
    min_best = []
    max_WIND =[]
    max_CROSS = []
    if len(temp_rows) != 0:
        minimum_CEILING = float(temp_rows[0][4])
        minimum_VISIBILITY = float(temp_rows[0][5])
       
        for row in temp_rows:
            if minimum_CEILING > float(row[4]):
                minimum_CEILING = float(row[4])
            if minimum_VISIBILITY > float(row[5]):
                minimum_VISIBILITY = float(row[5])
            max_WIND.append(float(row[6]))
            max_CROSS.append(float(row[7]))
        max_CROSS  = max(max_CROSS) 
        max_WIND = max(max_WIND) 
        min_best.append(minimum_CEILING)
        min_best.append(minimum_VISIBILITY)
        min_best.append(max_WIND)
        min_best.append(max_CROSS)
        print('best', min_best)
        return min_best
    else:
        return None
