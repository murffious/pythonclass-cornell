...
def grade_func1(file,step=0,outp=sys.stdout):
    ...
    for data in testcases:
        try:
            ...
            outp.write('\n'.join(error[:1]+error[3:5]+error[-2:]))
            outp.write('Reread the precondition for %s.\n\n' % repr(function))
            score -= 1/len(testcases)
            if not step:
                return (FAIL_INCORRECT,max(0,score))
...
def grade_func2(file,step=0,outp=sys.stdout):
    ...
    import pytz
    testcases = [('2015-06-05T07:00:00',True,True),  ('2015-06-05T17:00:00',True,True),
    ...
    for data in testcases:
        try:
            env.reset()        
            input = dateutil.parser.parse(data[0])
            input = eastern.localize(input)
