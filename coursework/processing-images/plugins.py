...
def mono(image, sepia):
    ...
    for row in range(height):
        for col in range(width):
            pixel = image[row][col]
            brightness = int(0.3 * pixel.red + 0.6 *
                             pixel.green + 0.1 * pixel.blue)
...
#     In image flipping you need to swap
#     the pixel values between two locations.
#     For example, in a horizontal flip,
#     the top-left pixel becomes the top-right pixel
#     and vice versa. While you could swap each of the
#     red, green, blue, and alpha values individually,
#     there is no problem with swapping the entire RGB
#     objects and leaving the contents of the objects
#     unchanged.
    height = len(image)
    width = len(image[0])
#     what is top left pixel?
    print(height, width)
    i = 1
    i_col = width
    for row in range(height):
        for col in range(width):
            print(col, row)
            pixel = image[row][col]
            pixel = image[row][col+height-i]
            i += 1
            if col == 28:

            print(row, col+height-i)
