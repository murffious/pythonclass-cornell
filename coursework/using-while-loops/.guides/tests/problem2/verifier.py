...
def verify_trace2(file,step,outp=sys.stdout):
    ...
    try:
        ...
        if env.module.__whileguard__ == env.LIMIT:
            ...
            outp.write('You have not limited the while-loop %s in to %d tries.\n' % (repr(function),debugLimit))
            return (FAIL_INCORRECT, 0)
        elif step:
            if len(env.printed) < debugLimit:
                ...
                outp.write("You have added extra debugging statements.  Only print out 'pos' at this step.\n")
                return (FAIL_INCORRECT, 0)
            else:
                miss = False
                for pos in range(len(env.printed)):
                    line = env.printed[pos]
                    if not str(output[pos]) in line:
                        miss = True
...
def grade_structure(file,func,step=0,outp=sys.stdout):
    ...
    if found4:
        outp.write('Function %s still has the limiting code, which should be removed' % repr(func))
        score -= 0.25
        if not step:
            return (FAIL_INCORRECT,max(0,score))
