            ...
            if env.inputed == []:
                ...
                if len(values[1:]) > 1 and env.printed == []:
                    ...
                    for pos in range(len(values[1:-1])):
                        item = values[pos+1]
                        if type(item) != int:
                            correct = "The bet must be a integer."
                        elif item <= 0:
                            correct = 'The bet must be positive.'
                        elif item > values[0]:
        ...
        if env.inputed == []:
            ...
            if len(env.printed) == 0:
                ...
                if not want in first and not 'score' in first:
                    outp.write("The call %s(%d) does display the score at the start.\n" % (function,args))
                    pos2 = 0
                    score -= 0.1
                    retry = True
                elif not first.startswith(correct):
                    outp.write("The call %s(%d) does display the score with the prefix %s.\n" % (function,args,repr(correct)))
                    score -= 0.05
                    retry = True
                elif not first.startswith(correct+want):
                    outp.write("The call %s(%d) does display the correct score at start [expected %s].\n" % (function,args,want))
                    score -= 0.05
                    retry = True
